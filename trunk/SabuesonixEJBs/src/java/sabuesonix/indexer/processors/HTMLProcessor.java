...
public class HTMLProcessor implements FileProcessor {
    ...
    /**
     * Decode the HTML file and set all words found.
     */
    private void decodeHTML() throws Exception {
        allWords = new LinkedList<Word>();
        URL url = new URL("file://" + document.getFile().getPath());
        StringExtractor extractor = new StringExtractor(url.getPath());
        String res = extractor.extractStrings(false);
